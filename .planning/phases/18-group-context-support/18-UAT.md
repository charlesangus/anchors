...
number: 1
name: Copy/paste inside a navigated Group
expected: |
  Enter a Group (double-click to navigate inside). Select a node that has an
  anchor pointing to it. Press Ctrl+C, then Ctrl+V. The pasted node should be
  replaced by a Link node wired to the anchor — and both the Link and the anchor
  should be inside the Group's nested DAG, not at root level.
