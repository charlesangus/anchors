...
def _find_host_main_window():
    """Return the host application's main window (a QMainWindow), or None.
...
# before the host main window is registered in topLevelWidgets(), but the
...
def _try_reparent_preloaded(widget, attempts_remaining):
    """Re-parent a parentless preloaded `widget` to the host's main window;
...
def launch(plugin, space_mode_order=None):
    ...
    if _tabtabtab_instance is not None:
        try:
            ...
            # before the host's main window existed in topLevelWidgets(),
            # _find_host_main_window() returned None and the cached
            ...
            if current_parent is None:
                rediscovered_parent = _find_host_main_window()
