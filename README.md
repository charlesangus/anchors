...
- **Scroll through all fuzzy-find results** — on by default. When checked, a search matching more anchors than fit in the picker keeps the rest and lets you reach them with the mouse wheel, the scrollbar, or the down arrow; when unchecked, only the first screenful of matches is kept (the historic behaviour). The picker is the same fixed size either way. Takes effect the next time a picker is opened, no restart needed.
